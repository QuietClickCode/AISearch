package com.aisearch.upms.client.shiro.filter;

import com.alibaba.fastjson.JSONObject;
import com.aisearch.common.util.PropertiesFileUtil;
import com.aisearch.common.util.RedisUtil;
import com.aisearch.upms.client.shiro.session.UpmsSessionDao;
import com.aisearch.upms.client.util.RequestParameterUtil;
...
/**
 * 重写authc过滤器
 * Created by shuaisearch on 2017/3/11.
 */
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    // 局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_ID = "aisearch-upms-client-session-id";
    // 单点同一个code所有局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_IDS = "aisearch-upms-client-session-ids";
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance("aisearch-upms-client").get("aisearch.upms.type");
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        StringBuffer ssoServerUrl = new StringBuffer(PropertiesFileUtil.getInstance("aisearch-upms-client").get("aisearch.upms.sso.server.url"));
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance("aisearch-upms-client").get("aisearch.upms.type");
        if ("server".equals(upmsType)) {
        ...
        ssoServerUrl.append("/sso/index").append("?").append("appid").append("=").append(PropertiesFileUtil.getInstance("aisearch-upms-client").get("aisearch.upms.appID"));
        // 回跳地址
