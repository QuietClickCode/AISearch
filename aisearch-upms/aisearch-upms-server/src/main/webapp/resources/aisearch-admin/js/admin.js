...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('aisearch-upms-systemid', systemid);
		$.cookie('aisearch-upms-systemname', systemname);
		$.cookie('aisearch-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('aisearch-upms-systemid') || 1;
	var systemname = $.cookie('aisearch-upms-systemname') || 'aisearch-upms-server';
	var systemtitle = $.cookie('aisearch-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
